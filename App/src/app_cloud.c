...
void USER_DataAnalysisProcess(char *RxBuf)
{
	...
	if(autoMode == 0) { //手动
		...
		printf("lightStatus:%d,lightPWM:%d\r\n",lightStatus,lightPWM);
		...
		if(sendFlag > 0){
			...
			if(LoRa_SendData_Affirm((uint8_t *)TxetBuf) == 0){//如果结点收到消息
				//MQTT数据回发
	...
	else if(strstr((const char *)RxBuf, (const char *)"set") != NULL)//命令请求？
	{
		#if 0
...
void USER_Datatoplayer(char *RxBuf)
{
	...
	{
		if(strstr((const char *)RxBuf, (const char *)"\"apitag\":\"string_play\"") != NULL)//语音播报
		{
			p_play = USER_GetJsonValue((char *)RxBuf, (char *)"data");
			DBG_B_INFO("%s\n", p_play);
   ...
   if(F_AT_RX_FINISH)//串口4接收到数据
	 ...
	 if((uint32_t)(SYSTICK_get_time()-lastTime>=1000))//10S发送一次数据,备注：该时间不准确，大约10s
	 {
		 ...
		 if(sendTypeFlag == 0){
			 ...
			 "fanStatus2","fanSpeed2", 
			 "motorStatus1", "motorSpeed1","motorDirection1",
			 "motorStatus2", "motorSpeed2","motorDirection2");
		 }
		 ...
		 if(sensorTimes >= 4){
			...
			if(onlineCode == 0) {//记录离线数据
				...
				allToJSON(12, msg, 
					...
					"s1", "s2",
					"fanStatus1", "fanSpeed1",
					"motorStatus1","onlineCode");//"infraredStatus1", "onlineCode");
